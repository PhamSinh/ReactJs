...
import { SEARCH_QUERY_PARAM_NAME } from "../search/constants";
...
export const useMoviesApiService = () => {
  ...
  const { status, error, data, isPreviousData } = useQuery(
    {
      queryKey: getQueryKey(page, query || undefined),
      queryFn: () => moviesApi.getMoviesByPageWithOptionalQuery(path, page, query || undefined)
    }
  );
